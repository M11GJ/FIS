...
科目情報を返すツールでは、次の項目も返します。

- `instructor`: 2026年度時間割に記載された担当教員。未公表の場合は `null`
- `room`: 講義室番号、`オンライン`、`オンデマンド`、`対面`などの時間割記載値。未公表の場合は `null`

担当教員と教室は、2026年9月11日版の全体時間割を優先し、情報科学部時間割、時間割変更一覧、集中講義一覧で補完したオフラインスナップショットです。MCP要求時に大学サイトをスクレイピングしません。休講・補講・教室変更は大学の最新案内を優先してください。

今回の更新に使用した資料は次のとおりです。SHA-256は同名資料の更新・差し替えを識別するために記録しています。

- `☆令和8年度全体版時間割：最終0911 (2).pdf`: `03ca1715d489d2099e0f7f1b1b63526f2dfc2487cd434c43afd5b14afbbef797`
- `【情報科学部】令和8年度時間割（2026.06.10更新） (2).pdf`: `e8af0ea12be5ddc0ac02b23ebb4bfb84a0fb52870de7344565a563b81d195e38`
- `時間割変更 (1).pdf`: `0d23c2fe34da0ce744e8487c088b2a1884abf3941d15347398775c0e47e1d376`
- `☆令和8年度時間割（集中講義） (1).pdf`: `7d4be37dc55a56b679715adedbb3c873c1bf3a33abdeda0e6f4fe0a187052f6a`
...
2026年度の情報科学部公式シラバス139件を確認し、構造化された「先修条件科目」の必須・推奨欄を出典URL・授業コード・確認日付きの静的スナップショットとして保持します。「なし」「特に無し」は条件なしとして除外し、自由文と構造化欄が矛盾する場合は構造化欄を優先します。38科目に必須条件または推奨条件があり、専門ゼミ1の1科目には科目名ではない進行条件があります。

`check_course_eligibility` の `completedPrerequisites` には、FISの科目一覧にない先修科目を修得済みとして申告できます。この値は先修条件の確認だけに使用し、修得単位・卒業判定には算入しません。FISの科目一覧にある科目名は無視して `ignoredCompletedPrerequisiteCatalogCourses` に返すため、通常の `completedCourses` へ指定してください。
...
- データベース応用: データベースが必須
- 金融工学: 微分積分基礎・確率統計基礎が必須
- 金融データ解析: 確率統計基礎・多変量解析が必須
- 応用数値解析: 数値解析が必須
- AI・画像情報処理: AI・機械学習基礎・ニューラルネットワークが必須
...
公式一覧と名称が一致しないFIS独自の集約科目は、先修条件を未検証と明示します。毎年度、公式シラバスをオフラインで照合してスナップショットを更新し、MCP要求中にActive Academyへスクレイピングは行いません。
